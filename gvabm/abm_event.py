...
def sample_event_with_meta(walk_radius, population_density, arm_density, atrisk_gathering_rate, rng_key):
    ...
    gathering_size = jnp.max(loc_gathering_size*location_mask, initial=0)
    rng_key, subkey = jrand.split(rng_key)
    return ((access_count > 0) & weapon_access & (location_count > 0)) * jrand.randint(subkey, (), minval=0, maxval=gathering_size), access_count, location_count
...
from gvabm.param_distr import CityParams, GeneralParams, MixedGeneralParams
