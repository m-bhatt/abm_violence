...
from stat_utils import CityParams, GeneralParams
def conditional_sample(city_params: CityParams, general_params: GeneralParams, rng_key, num_samples=10000, sample_func=batch_sample):
    population, population_density, arm_count, arm_density = city_params
    propensity, walk_radius, arm_bias, arm_weight, atrisk_gathering_rate = general_params
    # s = int(1e5)
    rng_batch = jrand.split(rng_key, num_samples)
    event_samples = sample_func(jnp.full(num_samples, walk_radius), jnp.full(num_samples, population_density), 
                                 jnp.full(num_samples, arm_density*arm_weight+arm_bias), 
                                 jnp.full(num_samples, atrisk_gathering_rate), rng_batch)
    pseudocount_samples = jnp.concatenate([event_samples, jnp.array([10, 100, 1000])])
    return pseudocount_samples

def sample_event_nopoisson(walk_radius, population_density, arm_density, atrisk_gathering_rate, rng_key):
    population_grid = jnp.full((30, 30), population_density)
    access_rate = arm_density * population_grid.size
    cell_access_rates = (population_grid / jnp.sum(population_grid)) * access_rate
    rng_key, subkey = jrand.split(rng_key)
    cell_access_counts = jrand.poisson(subkey, lam=cell_access_rates)
    access_grid = cell_access_counts > 0
    ...
    gathering_size = jnp.max(loc_gathering_size*location_mask, initial=0)
    rng_key, subkey = jrand.split(rng_key)
    return (weapon_access & (location_count > 0)) * jrand.uniform(subkey, (), minval=0, maxval=gathering_size)


batch_sample_nopoisson = jax.jit(jax.vmap(sample_event, 0), device=jax.devices()[0])
from stat_utils import CityParams, GeneralParams
def conditional_sample_nopoisson(city_params: CityParams, general_params: GeneralParams, rng_key, num_samples=10000):
    population, population_density, arm_count, arm_density = city_params
    propensity, walk_radius, arm_bias, arm_weight, atrisk_gathering_rate = general_params
    rng_batch = jrand.split(rng_key, num_samples)
    event_samples = batch_sample_nopoisson(jnp.full(num_samples, walk_radius), jnp.full(num_samples, population_density), 
                                 jnp.full(num_samples, arm_density*arm_weight+arm_bias), 
                                 jnp.full(num_samples, atrisk_gathering_rate), rng_batch)
    pseudocount_samples = jnp.concatenate([event_samples, jnp.array([10, 100, 1000])])
    return pseudocount_samples